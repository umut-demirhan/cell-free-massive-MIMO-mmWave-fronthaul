...
from sim_objects import State, uniform_square_rand_object, organized_square_object
...
def sample(rep_idx, params, save_folder='fig4'):
    ...
    state = State(cpu_loc=params['cpu_loc'], 
                      ...
                      fn_AP=organized_square_object,
                      wire_clusters=np.arange(100).reshape((10, 10)),
                      wire_cluster_heads = np.arange(0, 100, 10))       
    ...
    fh_group_parameters = channel_est[state.wire_clusters, :].sum(axis=1) # Select 
    for g in range(len(params['G'])):
        
        fh_groups = grouping.power_based_fixed_groups(fh_group_parameters, group_size=params['G'][g])
        ac_groups = np.zeros(channel_est.shape, dtype=bool)
        for ue in range(params['K']):
            ac_groups[state.wire_clusters[fh_groups[:, ue]], ue] = True
        R_ac = ac.rate_optimal_power(channel_est, groups=ac_groups)
        
        for fh_idx in range(len(fh_list)):
            fh = fh_list[fh_idx]
            SNR_fh = fh.optimize_group_SNR(groups=fh_groups) # Optimize BF Fronthaul
            for bw in  range(len(params['B_fh'])): # Fronthaul BW
                ...
                save_loc = os.path.abspath(os.path.join(save_folder, 'FH%i_Rep%i_BW%i_G%i.npz'%(fh_idx, rep_idx, bw, g)))
                np.savez(save_loc, *[fh_rates, ac_rates, fh_equal_rates, ac_groups, fh_groups, R_ac, R_fh])
...
if __name__=='__main__':
    ...
    params =     {'reps': 200, # Repetition of this simulation
                  'M': 100, # Number of APs
                  'K': 10, # Number of UEs
                  'L': 128, # Number of CPU antennas
                  
                  'G': np.arange(10)+1, # Group size, fully select if None
                  ...
                  'B_fh': np.array([20e6, 40e6, 60e6, 80e6, 100e6]), # Fronthaul BW: 1 GHz
    ...
    for i in range(len(list_BW)):
        color = colors(i)#next(colors)
        i = list_BW[i]
        plt.plot(params['G'], data_plot[2, i, :], '--', label='FH-%iMHz'%int(params['B_fh'][i]/1e6), c=color)
        plt.plot(params['G'], data_plot[0, i, :], '-.', label='E2E-%iMHz'%int(params['B_fh'][i]/1e6), c=color)
        #plt.plot(params['G'], fh_rate_equal_exact.mean(axis=0)[i, :], '-', c=color)
        
    plt.plot(params['G'], data_plot[1, 0, :], 'k:', label='Access Channel')
    plt.xlabel('Group Size - Clusters (G)')
