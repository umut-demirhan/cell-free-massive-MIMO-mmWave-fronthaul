...
if __name__=='__main__':
    ...
    params =     {'reps': 200, # Repetition of this simulation
                  ...
                  'G': np.arange(5, 51, 5), # Group size, fully select if None
                  ...
                  'B_fh': np.array([20e6, 40e6, 80e6, 120e6, 240e6, 480e6, 960e6]), # Fronthaul BW: 1 GHz
    ...
    for i in range(len(list_BW)):
        ...
        i = list_BW[i]
        plt.plot(params['G'], data_plot[0, i, :], '--', label='FH-%iMHz'%int(params['B_fh'][i]/1e6), c=color)
        plt.plot(params['G'], data_plot[2, i, :], '-.', label='End-to-End-%iMHz'%int(params['B_fh'][i]/1e6), c=color)
        #plt.plot(params['G'], fh_rate_equal_exact.mean(axis=0)[i, :], '-', c=color)
        
    plt.plot(params['G'], data_plot[1, 0, :], 'k:', label='AC')
